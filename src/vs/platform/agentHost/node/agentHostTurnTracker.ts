...
export class AgentHostTurnTracker extends Disposable {
	...
	turnCompleted(session: string, turnId: string, result: AgentHostTurnResult, failure?: IAgentHostTurnFailure, workspace?: { readonly isMultiRoot: boolean; readonly workingDirectoryCount: number }): void {
		const key = this._key(session, turnId);
		...
		this._reporter.turnCompleted({
			...
			workingDirectoryCount: workspace?.workingDirectoryCount ?? 0,
		});
	}
