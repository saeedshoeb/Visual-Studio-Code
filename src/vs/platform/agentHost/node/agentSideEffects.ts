...
export class AgentSideEffects extends Disposable {
	...
	private _completeTurn(channel: string, turnId: string, result: AgentHostTurnResult, failure?: IAgentHostTurnFailure): void {
		...
		const workingDirectoryCount = this._agentConfigService.getEffectiveWorkingDirectories(sessionUri)?.length ?? 0;
		this._turnTracker.turnCompleted(channel, turnId, result, failure, { isMultiRoot: workingDirectoryCount > 1, workingDirectoryCount });
	}
